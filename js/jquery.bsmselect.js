 ...
 *
 *
 * CHANGED 2013-08-22 : it is now allowed to select an item more than once
 */
...
(function($) {
  ...
  BsmSelect.prototype = {
    ...
    originalChangeEvent: function() {
      ...
      ////CHANGED - ADDED PART BELOW
      //Update global hidden input (to pass info to a formulary)
      aux = this.$list.children();

      document.getElementById('all_files').value = "";
      aux_array = [];

      aux.each(function() {
        aux_array.push($(this)[0].getAttribute('data-extra'));
      });

      document.getElementById('all_files').value = aux_array.join("|");
    },
     ...
     */
     ...
     var $bsmOpt = $('<option>', {
      ...
      if (isSelected && !isDisabled) {
        this.addListItem($bsmOpt);
        //this.disableSelectOption($bsmOpt); ////CHANGED
    ...
    addListItem: function($bsmOpt) {
      ...
      $item = $('<li>', { 'class': o.listItemClass, 'data-extra'  : $bsmOpt[0].value})
        .append($('<a>', { href: '#', 'class': o.removeClass, html: o.removeLabel })) //CHANGED ORDER
        .append($('<span>', { 'class': o.listItemLabelClass, html: o.extractLabel($bsmOpt, o)}))
        .data('bsm-option', $bsmOpt);

      //this.disableSelectOption($bsmOpt.data('item', $item)); ////CHANGED
...
*/
          ////CHANGED - ADDED PART BELOW
          //Update global hidden input (to pass info to a formulary)
          aux = $("#mp3files").data("bsmSelect").$list.children();
